    ...
    QObject( parent )
{
    ...
    m_tray->setTitle( tr ( "Ctlos Settings Manager" ) );
    m_tray->setIconByName( "ctlos-settings-manager" );

    QMenu* menu = m_tray->contextMenu();

    QAction* msmKernel = new QAction( QIcon( ":/icons/tux-ctlos.png" ),
                                      tr ( "Kernels" ),
                                      menu );
    ...
    connect( msmKernel, &QAction::triggered,
             [this] ()
    {
        QProcess::startDetached( "ctlos-settings-manager", QStringList() << "-m" << "msm_kernel" );
        m_tray->setStatus( KStatusNotifierItem::Passive );
    } );
    connect( msmLanguagePackages, &QAction::triggered,
             [this] ()
    {
        QProcess::startDetached( "ctlos-settings-manager", QStringList() << "-m" << "msm_language_packages" );
        m_tray->setStatus( KStatusNotifierItem::Passive );
    } );
...
Notifier::cLanguagePackage()
{
    ...
    if ( !packages.isEmpty() )
    {
        ...
        m_tray->showMessage( tr( "Ctlos Settings Manager" ),
...
{
    QSettings settings( "ctlos", "ctlos-settings-manager" );
...
{
    QSettings settings( "ctlos", "ctlos-settings-manager-Notifier" );
