...
{
    return QString( tr ( "Add and remove Ctlos kernels" ) );
}
...
KernelCommon::installKernel( const QModelIndex& index )
{
    ...
    KAuth::Action installAction( QLatin1String( "org.ctlos.msm.kernel.install" ) );
    installAction.setHelperId( QLatin1String( "org.ctlos.msm.kernel" ) );
...
KernelCommon::removeKernel( const QModelIndex& index )
{
    ...
    QVariantMap args;
    args["arguments"] = arguments;
    KAuth::Action installAction( QLatin1String( "org.ctlos.msm.kernel.remove" ) );
    installAction.setHelperId( QLatin1String( "org.ctlos.msm.kernel" ) );
    installAction.setArguments( args );
    installAction.setTimeout( std::numeric_limits<int>::max() );
